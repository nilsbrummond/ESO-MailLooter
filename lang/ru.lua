...
ZO_CreateStringId("SI_MAILLOOTER_TITLE", "MailLooter")
...
ZO_CreateStringId("SI_MAILLOOTER_FILTER_LABEL_PROVISION",	"Снабженец")
...
ZO_CreateStringId("SI_MAILLOOTER_OVERVIEW_AUTO_RTN", "Автовозврат:")
...
"Все параметры на этой странице сохраняются как общие на аккаунт и будут применяться для всех Ваших персонажей.  Только фильтры сбора на закладке MailLooter (не на этой странице) сохраняются отдельно для каждого персонажа.")
...
"Считать письмо простым, если кол-во слов в теме меньше либо равно N.  Письма, содержащие в теме слова из списка автовозврата, указанные ниже, простыми не считаются.")
...
ZO_CreateStringId("SI_MAILLOOTER_SETTINGS_AR_TITLE","Автовозврат отправителю")
ZO_CreateStringId("SI_MAILLOOTER_SETTINGS_AR_DECT",	"Определение писем для автовозврата")
ZO_CreateStringId("SI_MAILLOOTER_SETTINGS_AR_DECT_DESC",
"Можно задать до 6 разных слов или фраз.  Если какое(ие)-нибудь из них присутствуют в теме письма, MailLooter будет считать, что такое письмо надо автоматически вернуть отправителю.  Во время сравнения с заголовком письма заглавные и прописные буквы не различаются, пунктуация игнорируется и лишние пробелы также игнорируются.  Оставьте поле 'Тема N' пустым, чтобы отключить его.  Аддон будет либо игнорировать, либо автоматически возвращать соответствующие письма.")
ZO_CreateStringId("SI_MAILLOOTER_SETTINGS_AR_EXAMPLES","Примеры:")
ZO_CreateStringId("SI_MAILLOOTER_SETTINGS_AR_EX1_T","1. Использовать другой аддон для автовозврата")
ZO_CreateStringId("SI_MAILLOOTER_SETTINGS_AR_EX1_DESC",
"Убедитесь, что настройка 'Разрешить автовозврат писем' ОТКЛЮЧЕНА.  Заполните 6 полей 'Тема N' словами, по которым другой аддон определяет автовозвращаемые письма.  MailLooter проигнорирует эти письма, чтобы не мешать работе другого аддона.")
ZO_CreateStringId("SI_MAILLOOTER_SETTINGS_AR_EX2_T","2. Использовать этот аддон для автовозврата")
ZO_CreateStringId("SI_MAILLOOTER_SETTINGS_AR_EX2_DESC",
"Убедитесь, что настройка 'Разрешить автовозврат писем' ВКЛЮЧЕНА.  Заполните 6 полей 'Тема N' словами, по которым вы хотите определять автовозвращаемые письма.  MailLooter автоматически вернет отправителю эти письма во время выполнения сбора почты.")
ZO_CreateStringId("SI_MAILLOOTER_SETTINGS_AR_EX3_T","3. Не выполнять автовозврат писем")
ZO_CreateStringId("SI_MAILLOOTER_SETTINGS_AR_EX3_DESC",
"Убедитесь, что настройка 'Разрешить автовозврат писем' ОТКЛЮЧЕНА.  Оставьте 6 полей 'Тема N' пустыми.  MailLooter не будет автоматически возвращать какие-либо письма, также никакие письма не будут считаться подлежащими автовозврату.")
ZO_CreateStringId("SI_MAILLOOTER_SETTINGS_AR_SUBJECT","Тема <<1>>:")
ZO_CreateStringId("SI_MAILLOOTER_SETTINGS_AR_ENABLE","Разрешить автовозврат писем")
...
ZO_CreateStringId("SI_MAILLOOTER_LOOT_MN_COD_PAYMENT",	"Оплата за COD")
